...
public class BorrowServlet extends HttpServlet {
	...
	private void addBorrow(HttpServletRequest request,
		...
		if(realBorrowNumber > 5){
			ret.put("type", "error");
			ret.put("msg", "借阅数量最多不能超过5本!");
			StringUtil.writrToPage(response, JSONObject.toJSONString(ret));
			return;
		}
		...
		if(isOver(book, loginUser)){
			ret.put("type", "error");
			ret.put("msg", "借阅总数不能超过5本，请先归还后再借阅!");
			StringUtil.writrToPage(response, JSONObject.toJSONString(ret));
			return;
		}
...
//设置归还期限
			java.util.Date borrowtime = borrow.getBorrowTime();
			long borrowtime2 = borrowtime.getTime();
			long returntime = new java.util.Date().getTime();
		    int days = (int) ((returntime - borrowtime2) / (1000));
			if ((days) >= 1) {
				User loginUser = (User)request.getSession().getAttribute("user");
				loginUser.setStatus(0);
				ret.put("type", "error");
				ret.put("msg", "超过了归还期限，您的状态被禁用，请联系管理员！");
				StringUtil.writrToPage(response, JSONObject.toJSONString(ret));
				return;
			}
		//判断是全部归还还是部分归还
	...
	private boolean isOver(Book book,User user){
		Page<Borrow> page = new Page<Borrow>(1, 999);
		page.getSearchProporties().add(new SearchProperty("user_id", user.getId(), Operator.EQ));
		page.getSearchProporties().add(new SearchProperty("status", 1, Operator.EQ));
		page = borrowDao.findList(page);
